...
def ethical_filter(text):
    banned_words = ["fucking", "shit", "bastard"]
    return not any(word in text.lower() for word in banned_words)

def is_hateful(text):
    hateful_keywords = ["kill", "hate", "nazi"]
    return any(word in text.lower() for word in hateful_keywords)

def contains_bias(text):
    biased_keywords = ["always", "never", "everyone"]
    return any(word in text.lower() for word in biased_keywords)

def grammar_ok(text):
    return text[0].isupper() and text.endswith(".")

def no_exaggeration(text):
    exaggerated = ["best movie ever", "incredible beyond words"]
    return not any(phrase in text.lower() for phrase in exaggerated)
